...
//
// RetroPanel frontpanel (fp) GPIO/Pin assignments
//
...
/* PinID  ARD  GPIO */
/*   52   D12   PB1 */  #define   fp_HALT     D12
/*   52   D13   PB1 */  #define   fp_RUN      D13

// End Frontpanel GPIO/Pin Definitions
...
void loop() {
  ...
  //   }
    ...
    // if (!done) {
      ...
      if (resetCnt > 0) {
        resetCnt--; // Count down number of cycles in reset
        if (resetCnt == 0)
          pio_setPin(uP_RST);
      }
...
void uP_initiate_reset() {
  ...
  resetCnt = 25;           // Reset uP for 25 cycles
}
...
void uP_tick() {
  ...
  RUN = pio_readPin(fp_RUN);

  if(RUN)
    pio_resetPin(uP_RDY); // Drive RDY low
  else
    pio_setPin(uP_RDY); // Drive RDY high
    

  pio_setPin(uP_CLK0); // Drive CLK high

  RST = pio_readPin(uP_RST); // Read the R/W pin
  RDY = pio_readPin(uP_RDY) ;
...
// #if SingleStepMode != 3
  char msg[80];
  sprintf(msg, "reset: %02d,  RST: %01X  RDY: %01X  SYNC:  %01X  IR:  %02X  R/W:  %01X  ADDR:  %04X  DATA:  %02X >", resetCnt, RST, RDY, SYNC, IR, RW, ADDR, DATA);
  Serial.println(msg);

//   if (SingleStepMode == 0 || SingleStepMode == 1 && resetCnt == 0) {
